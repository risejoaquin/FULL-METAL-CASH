...
```powershell
Unblock-File .\scripts\ga\validate-lga-11-limited-ga-public-ga-readiness-reassessment.ps1
Unblock-File .\scripts\security\scan-local-secrets.ps1
Unblock-File .\scripts\posdashboard\validate-posdashboard-operations-dashboard.ps1
```

## 3. Execute LGA-11 — Limited GA Public GA Readiness Reassessment

```powershell
.\scripts\ga\validate-lga-11-limited-ga-public-ga-readiness-reassessment.ps1 `
  ...
  -WpfVisualConfirmed `
  -SkipDashboardBuild `
  -SkipLga10Revalidation
```

Use `-SkipLga10Revalidation` only because LGA-10 already has reviewed PASS production logs. Remove it if the LGA-10 runtime manifest exists locally and you want prerequisite manifest enforcement.

## Expected PASS with current capacity state

```text
[LGA-11] PASS LGA-11 LIMITED GA PUBLIC GA READINESS REASSESSMENT / KEEP LIMITED GA / PUBLIC GA NOT ACTIVATED
```

## If it fails
Send the complete PowerShell output beginning at `[LGA-11] Validator version` through the blocker/exception and final manifest values. Do not send passwords, JWTs, `DATABASE_URL`, access tokens or Authorization headers.
