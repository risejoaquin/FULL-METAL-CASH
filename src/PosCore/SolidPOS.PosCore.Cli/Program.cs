...
switch (command)
{
    ...
    case "create-update-package":
    {
        ...
        var tenantName = GetOption(args, "--tenant-name");
        var appName = GetOption(args, "--app-name");
        var releaseVersion = GetOption(args, "--release-version", "1.0.0");
        var channel = GetOption(args, "--channel", "stable");
        var packagePath = GetOption(args, "--output-package");
        var manifestPath = GetOption(args, "--output-manifest");
        var brandingPackagePath = GetOption(args, "--branding-package", string.Empty);
        var minimumPosCoreVersion = GetOption(args, "--minimum-poscore-version", "1.0.0");
        var minimumPosBuilderVersion = GetOption(args, "--minimum-posbuilder-version", "1.0.0");
        var notes = GetOption(args, "--notes", "SolidPOS local update package generated by PosBuilder.");

        var packageDirectory = Path.GetDirectoryName(packagePath);
        if (!string.IsNullOrWhiteSpace(packageDirectory)) Directory.CreateDirectory(packageDirectory);
        var manifestDirectory = Path.GetDirectoryName(manifestPath);
        if (!string.IsNullOrWhiteSpace(manifestDirectory)) Directory.CreateDirectory(manifestDirectory);

        var brandingPackageVersion = TenantBrandingPackage.CurrentPackageVersion;
        if (!string.IsNullOrWhiteSpace(brandingPackagePath) && File.Exists(brandingPackagePath))
        {
            var brandingService = new TenantBrandingPackageService(new JsonTenantBrandingPackageStore());
            var brandingPackage = await brandingService.LoadValidatedAsync(brandingPackagePath).ConfigureAwait(false);
            brandingPackageVersion = brandingPackage.PackageVersion;
        }

        var packageLines = new[]
        {
            "SolidPOS local update package",
            $"tenantId={tenantId}",
            $"tenantName={tenantName}",
            $"appName={appName}",
            $"releaseVersion={releaseVersion}",
            $"channel={channel}",
            $"brandingPackageVersion={brandingPackageVersion}",
    ...
    case "show-update-package":
    {
        var manifestPath = GetOption(args, "--manifest");
        var service = new UpdatePackageManifestService(new JsonUpdatePackageManifestStore());
        var manifest = await service.LoadValidatedAsync(manifestPath).ConfigureAwait(false);
        Console.WriteLine($"Update package manifest. tenantId={manifest.TenantId}; appName={manifest.AppName}; releaseVersion={manifest.ReleaseVersion}; channel={manifest.Channel}; packageFile={manifest.PackageFileName}; sizeBytes={manifest.PackageSizeBytes}; sha256={manifest.Sha256}; minimumPosCoreVersion={manifest.MinimumPosCoreVersion}; minimumPosBuilderVersion={manifest.MinimumPosBuilderVersion}; brandingPackageVersion={manifest.BrandingPackageVersion}");
        return 0;
    }
