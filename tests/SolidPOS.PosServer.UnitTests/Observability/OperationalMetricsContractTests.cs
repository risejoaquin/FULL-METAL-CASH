...
public sealed class OperationalMetricsContractTests
{
    ...
    public void Metrics_contract_exposes_hardening_signals()
    {
        ...
        Assert.Equal(0, response.Database.ActiveNonClientWaitEventCount);
        Assert.Equal(4, response.Database.ClientReadWaitEventCount);
        Assert.Equal(2, response.Inventory.NegativeInventoryItemCount);
        Assert.Equal(0, response.FinancialIntegrity.SalePaymentMismatchCount);
