...
Write-Host "Validating PosDashboard Reports / Audit / Operations dashboard..."
...
try {
  ...
  Write-Host "Building dashboard production bundle..."
  Invoke-CheckedCommand -FilePath "npm" -Arguments @("run", "build") -FailureMessage "Dashboard build failed."

  Write-Host "Running dashboard operations self-test..."
