param(
    ...
    [string]$CorrectionReason = 'LGA-07.3 controlled negative stock regression correction after real POS activity',
    [switch]$SkipDashboardBuild
)
$ErrorActionPreference='Stop'
$script:ValidatorVersion='LGA-07.3-negative-stock-regression-diagnostics-and-correction-no-public-ga'
function Write-Step([string]$m){Write-Host "[LGA-07.3] $m"}
...
$sqlPath=Join-Path $scriptRoot 'lga-07-hotfix-07-3-negative-stock-regression-diagnostics-and-correction-check.sql'
$runtimeDir=Join-Path $repo '.runtime\lga-07-hotfix-07-3-negative-stock-regression-diagnostics-and-correction'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:ValidatorVersion"
Write-Step 'Repository/document HOTFIX LGA-07.3 guardrails...'
...
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-07-hotfix-07-3-negative-stock-regression-diagnostics-and-correction.md') @('lga-07.3','negative stock regression','inventory correction','diagnostic first','applyinventorycorrection','public ga not activated','no baseline increase')
Write-Step 'Repository/document HOTFIX LGA-07.3 guardrails PASS'
...
$db=Invoke-DbJsonFile $sqlPath @{tenant_id=$TenantId} 'LGA073_INVENTORY_JSON:'
...
$recommendedAction=if($negativeCount -gt $AllowedNegativeStockItemCount){'Rerun this hotfix with -ApplyInventoryCorrection after reviewing the negative stock item details. Do not rerun LGA-07 until negativeStockCount is 0.'}else{'Rerun LGA-07 continued monitoring validator with AllowedNegativeStockItemCount 0 and AllowedWaitingConnectionCount 12.'}
...
$status=if($negativeCount -gt $AllowedNegativeStockItemCount){'PASS HOTFIX LGA-07.3 NEGATIVE STOCK REGRESSION DIAGNOSTICS / CORRECTION REQUIRED BEFORE LGA-07'}else{'PASS HOTFIX LGA-07.3 NEGATIVE STOCK REGRESSION CORRECTION / RERUN LGA-07'}
$manifest=[ordered]@{
  phase='LGA-07.3'
  status=$status
  ...
  nextPhase=if($negativeCount -gt $AllowedNegativeStockItemCount){'Apply LGA-07.3 correction, then rerun LGA-07'}else{'Rerun LGA-07 continued monitoring validator'}
}
$manifestPath=Join-Path $runtimeDir ("lga-07-hotfix-07-3-negative-stock-regression-"+(Get-Date).ToString('yyyy-MM-dd-HHmmss')+'.json')
