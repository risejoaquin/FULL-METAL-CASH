param(
    ...
    [ValidateSet('CONTINUED_MONITORING','POST_UPGRADE_VERIFICATION')][string]$VerificationMode = 'CONTINUED_MONITORING',
    ...
    [switch]$SkipLga07Revalidation
)
$ErrorActionPreference='Stop'
$script:Lga08ValidatorVersion='LGA-08.0-limited-ga-post-upgrade-verification-or-continued-monitoring'
function Write-Step([string]$m){Write-Host "[LGA-08] $m"}
...
$sln=Join-Path $repo 'solidpos-platform.sln';$sqlPath=Join-Path $scriptRoot 'lga-08-limited-ga-post-upgrade-verification-or-continued-monitoring-check.sql'
...
$runtimeDir=Join-Path $repo '.runtime\lga-08-limited-ga-post-upgrade-verification-or-continued-monitoring';$lga07RuntimeDir=Join-Path $repo '.runtime\lga-07-limited-ga-capacity-upgrade-execution-or-continued-monitoring'
$wpfSalesVm=Join-Path $repo 'src\PosCore\SolidPOS.PosCore.Wpf\ViewModels\SalesViewModel.cs'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:Lga08ValidatorVersion"
Write-Step 'Repository/document LGA-08 guardrails...'
Assert-True (Test-Path $sln) 'solidpos-platform.sln missing';Assert-True (Test-Path $sqlPath) 'LGA-08 SQL check missing'
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-08-limited-ga-post-upgrade-verification-or-continued-monitoring.md') @('lga-07','post-upgrade verification','continued monitoring','public ga not activated','schema version 4')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-08-post-upgrade-verification-plan.md') @('post-upgrade verification','capacity probe','rollback','limited ga','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-08-continued-monitoring-checklist.md') @('continued monitoring','negative stock','waiting connections','sync conflict','dead letter','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-08-evidence-matrix.md') @('evidence matrix','lga-07','schema version 4','capacity','decision')
Write-Step 'Repository/document LGA-08 guardrails PASS'

Write-Step 'LGA-07 prerequisite...'
if($SkipLga07Revalidation.IsPresent){Write-Step 'LGA-07 prerequisite revalidation skipped by switch; LGA-08 requires existing LGA-07 PASS logs with CONTINUE LIMITED GA.'}
else{
  Assert-True (Test-Path $lga07RuntimeDir) 'LGA-07 runtime manifest directory missing. Rerun LGA-07 or use -SkipLga07Revalidation only if PASS logs are available.'
  $lga07Manifests=@(Get-ChildItem -Path $lga07RuntimeDir -Filter '*.json' -ErrorAction SilentlyContinue)
  $lga07Pass=$false
  foreach($f in $lga07Manifests){try{$m=Get-Content -Raw $f.FullName|ConvertFrom-Json;if(([string](Req $m 'status')).Contains('PASS LGA-07 LIMITED GA CAPACITY UPGRADE EXECUTION OR CONTINUED MONITORING')){$lga07Pass=$true}}catch{}}
  Assert-True $lga07Pass 'LGA-07 PASS manifest not found. LGA-08 requires LGA-07 closure.'
}
Write-Step 'LGA-07 prerequisite PASS'
...
Write-Step 'LGA-08 post-upgrade/continued-monitoring API checks...'
...
Write-Step 'LGA-08 post-upgrade/continued-monitoring API checks PASS'
...
Write-Step 'Database LGA-08 verification snapshot...'
$db=Invoke-DbJsonFile $sqlPath @{tenant_id=$TenantId;max_stores=$MaxStores;max_concurrent_terminals=$MaxConcurrentTerminals;allowed_existing_sync_conflicts=$AllowedExistingSyncConflictCount;allowed_dead_letters=$AllowedDeadLetterCount;allowed_waiting_connections=$AllowedWaitingConnectionCount} 'LGA08_JSON:'
...
Write-Step 'Database LGA-08 verification snapshot PASS'

Write-Step 'LGA-08 limited GA capacity upgrade execution or continued monitoring blocker matrix...'
