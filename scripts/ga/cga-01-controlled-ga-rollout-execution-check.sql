...
\set observation_window_hours_value :observation_window_hours
WITH required_tables AS (
  ...
  SELECT
    ...
    :'observation_window_hours_value'::int AS observation_window_hours
), rollout_activity AS (
  SELECT
    (SELECT count(*)::bigint FROM pos.sales WHERE tenant_id = :'tenant_uuid'::uuid AND status = 'completed' AND occurred_at >= now() - interval '7 days') AS completed_sales_last_7_days,
    (SELECT COALESCE(sum(total_cents),0)::bigint FROM pos.sales WHERE tenant_id = :'tenant_uuid'::uuid AND status = 'completed' AND occurred_at >= now() - interval '7 days') AS completed_sales_cents_last_7_days,
    (SELECT count(*)::bigint FROM pos.payments WHERE tenant_id = :'tenant_uuid'::uuid AND created_at >= now() - interval '7 days') AS payments_last_7_days,
    (SELECT count(*)::bigint FROM pos.digital_receipts WHERE tenant_id = :'tenant_uuid'::uuid AND issued_at >= now() - interval '7 days') AS receipts_issued_last_7_days
), sync_integrity AS (
  SELECT
  ...
  SELECT jsonb_build_object(
    ...
    'rolloutActivity', (SELECT row_to_json(rollout_activity) FROM rollout_activity),
...
SELECT 'CGA01_JSON:' || result::text FROM payload;
