param(
    ...
    [ValidateSet('APPROVE_PUBLIC_GA_ACTIVATION','DEFER_PUBLIC_GA_ACTIVATION','NO_GO')][string]$ActivationDecision = 'APPROVE_PUBLIC_GA_ACTIVATION',
    [ValidateSet('KEEP_NOT_ACTIVATED')][string]$ExecutionDecision = 'KEEP_NOT_ACTIVATED',
    [int]$MaxStores = 2,
    ...
    [switch]$SkipPublicGaReadinessReviewRevalidation
)
$ErrorActionPreference='Stop'
$script:PublicGaActivationDecisionValidatorVersion='PUBLIC-GA-ACTIVATION-DECISION.1.0-final-go-no-go-not-execution'
function Write-Step([string]$m){Write-Host "[PUBLIC-GA-ACTIVATION-DECISION] $m"}
...
$sln=Join-Path $repo 'solidpos-platform.sln';$sqlPath=Join-Path $scriptRoot 'public-ga-activation-decision-check.sql'
...
$runtimeDir=Join-Path $repo '.runtime\public-ga-activation-decision';$readinessRuntimeDir=Join-Path $repo '.runtime\public-ga-readiness-review'
$wpfSalesVm=Join-Path $repo 'src\PosCore\SolidPOS.PosCore.Wpf\ViewModels\SalesViewModel.cs'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:PublicGaActivationDecisionValidatorVersion"
Write-Step 'Repository/document PUBLIC GA ACTIVATION DECISION guardrails...'
Assert-True (Test-Path $sln) 'solidpos-platform.sln missing';Assert-True (Test-Path $sqlPath) 'Public GA activation decision SQL check missing'
Assert-DocumentContains (Join-Path $repo 'docs\ga\public-ga-activation-decision.md') @('public ga','activation decision','go/no-go','not activated','explicit approval','rollback')
Assert-DocumentContains (Join-Path $repo 'docs\ga\public-ga-activation-decision-checklist.md') @('security','disaster recovery','observability','capacity','rollback','activation not executed')
Assert-DocumentContains (Join-Path $repo 'docs\ga\public-ga-activation-decision-evidence-matrix.md') @('ga-07','ga-08','ga-10','ga-11','ga-12','lga-12','post-lga capacity','public ga readiness review')
Assert-DocumentContains (Join-Path $repo 'docs\ga\public-ga-activation-execution-boundary.md') @('separate execution','explicit user instruction','not activated','rollback','abort')
...
Write-Step 'Repository/document PUBLIC GA ACTIVATION DECISION guardrails PASS'

Write-Step 'PUBLIC GA READINESS REVIEW prerequisite...'
if($SkipPublicGaReadinessReviewRevalidation.IsPresent){Write-Step 'PUBLIC GA READINESS REVIEW prerequisite revalidation skipped by switch; reviewed PASS production logs are required.'}
else{
  Assert-True (Test-Path $readinessRuntimeDir) 'PUBLIC GA READINESS REVIEW runtime manifest directory missing. Rerun readiness review or use -SkipPublicGaReadinessReviewRevalidation only with reviewed PASS logs.'
  $readinessManifests=@(Get-ChildItem -Path $readinessRuntimeDir -Filter '*.json' -ErrorAction SilentlyContinue)
  $readinessPass=$false
  foreach($f in $readinessManifests){try{$m=Get-Content -Raw $f.FullName|ConvertFrom-Json;if(([string](Req $m 'status')).Contains('PASS PUBLIC GA READINESS REVIEW') -and ([string](Req $m 'readinessDecision')) -eq 'RECOMMEND_PUBLIC_GA_GO'){$readinessPass=$true}}catch{}}
  Assert-True $readinessPass 'Reviewed PUBLIC GA READINESS REVIEW PASS manifest not found.'
}
Write-Step 'PUBLIC GA READINESS REVIEW prerequisite PASS'
...
Write-Step 'Database Public GA activation decision snapshot...'
...
Write-Step 'Database Public GA activation decision snapshot PASS'

Write-Step 'Public GA activation decision GO/NO-GO blocker matrix...'
...
if($ExecutionDecision -ne 'KEEP_NOT_ACTIVATED'){Add-Blocker 'execution_decision_must_keep_public_ga_not_activated' $ExecutionDecision}
if($ActivationDecision -eq 'APPROVE_PUBLIC_GA_ACTIVATION' -and -not $capacityProbePassed){Add-Blocker 'activation_approval_requires_capacity_gate_pass' $ActivationDecision}
Add-Condition 'public_ga_not_activated';Add-Condition 'public_ga_readiness_review_passed';Add-Condition 'capacity_gate_passed';Add-Condition ('activation_decision_' + $ActivationDecision.ToLowerInvariant());Add-Condition 'activation_execution_separate';Add-Condition 'schema_version_4_required';Add-Condition 'negative_stock_zero_required';Add-Condition 'waiting_connections_baseline_not_raised';Add-Condition 'sync_conflict_and_dead_letter_baselines_carried_forward';Add-Condition 'readiness_single_catalog_query_required'
Assert-True ($blockers.Count -eq 0) ("PUBLIC-GA-ACTIVATION-DECISION blockers present: " + ($blockers|ConvertTo-Json -Depth 20 -Compress))
Write-Step 'Public GA activation decision GO/NO-GO blocker matrix PASS'

$status=if($ActivationDecision -eq 'APPROVE_PUBLIC_GA_ACTIVATION'){'PASS PUBLIC GA ACTIVATION DECISION / GO APPROVED / ACTIVATION EXECUTION AUTHORIZED / PUBLIC GA NOT ACTIVATED'}elseif($ActivationDecision -eq 'DEFER_PUBLIC_GA_ACTIVATION'){'PASS PUBLIC GA ACTIVATION DECISION / DEFER RECORDED / PUBLIC GA NOT ACTIVATED'}else{'PASS PUBLIC GA ACTIVATION DECISION / NO-GO RECORDED / PUBLIC GA NOT ACTIVATED'}
$nextPhase=if($ActivationDecision -eq 'APPROVE_PUBLIC_GA_ACTIVATION'){'PUBLIC_GA_ACTIVATION_EXECUTION_AUTHORIZED_NOT_EXECUTED'}elseif($ActivationDecision -eq 'DEFER_PUBLIC_GA_ACTIVATION'){'PUBLIC_GA_ACTIVATION_DEFERRED'}else{'PUBLIC_GA_REMEDIATION_REQUIRED'}
$manifest=[ordered]@{phase='PUBLIC-GA-ACTIVATION-DECISION';status=$status;tenantId=$TenantId;baseUrl=$BaseUrl;dashboardUrl=$DashboardUrl;generatedAt=(Get-Date).ToUniversalTime().ToString('o');checkpointDate=(Get-Date).ToString('yyyy-MM-dd');validatorVersion=$script:PublicGaActivationDecisionValidatorVersion;entryGate='PASS PUBLIC GA READINESS REVIEW / GO RECOMMENDATION AUTHORIZED';activationDecision=$ActivationDecision;executionDecision=$ExecutionDecision;publicGeneralAvailabilityActivated=$false;generalAvailabilityActivated=$false;publicGaActivatedByValidator=$false;publicGaReadinessConcurrency=$PublicGaReadinessConcurrency;concurrencyProbeRequests=$ConcurrencyProbeRequests;maxReadinessP95Ms=$MaxReadinessP95Ms;capacityProbePassed=$capacityProbePassed;healthLiveConcurrencyProbe=$liveProbe;healthReadyConcurrencyProbe=$readyProbe;healthLiveStatus=$healthLiveStatus;healthReadyStatus=$healthReadyStatus;waitingConnectionCount=$waitingConnectionCount;longRunningQueryCount=$longRunningQueryCount;negativeStockCount=$negativeStockCount;activeStoreCount=$activeStoreCount;availableTerminalCount=$availableTerminalCount;completedSalesInLast24h=$completedSales24h;paymentsInLast24h=$payments24h;receiptsIssuedInLast24h=$receiptsIssued24h;auditEventsInLast24h=$auditEvents24h;openShiftCount=$openShiftCount;closedShiftCount=$closedShiftCount;cashDifferenceLast24hCount=$cashDifferenceLast24hCount;syncPendingCount=$syncPendingCount;syncProcessingCount=$syncProcessingCount;syncRetryPendingCount=$syncRetryPendingCount;syncConflictCount=$syncConflictCount;syncDeadLetterCount=$syncDeadLetterCount;databaseSnapshot=$db;blockerCount=$blockers.Count;blockers=$blockers;conditions=@($conditions);schemaVersion=4;syncContract='schema_version_4';readinessSqlCommandsPerRequest=1;publicGaActivation='NOT_ACTIVATED';activationExecuted=$false;nextPhase=$nextPhase}
$manifestPath=Join-Path $runtimeDir ("public-ga-activation-decision-" + (Get-Date).ToString('yyyy-MM-dd-HHmmss') + ".json");$manifest|ConvertTo-Json -Depth 100|Set-Content -Encoding UTF8 $manifestPath
Write-Step 'PUBLIC-GA-ACTIVATION-DECISION evidence manifest PASS'
Write-Step $status
