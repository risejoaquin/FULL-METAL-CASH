param(
    ...
    [ValidateSet('CONTINUE_LIMITED_GA')][string]$CommercialDecision = 'CONTINUE_LIMITED_GA',
    ...
    [switch]$SkipLga09Revalidation
)
$ErrorActionPreference='Stop'
$script:Lga10ValidatorVersion='LGA-10.0-limited-ga-commercial-operations-confidence-gate'
function Write-Step([string]$m){Write-Host "[LGA-10] $m"}
...
$sln=Join-Path $repo 'solidpos-platform.sln';$sqlPath=Join-Path $scriptRoot 'lga-10-limited-ga-commercial-operations-confidence-gate-check.sql'
...
$runtimeDir=Join-Path $repo '.runtime\lga-10-limited-ga-commercial-operations-confidence-gate';$lga09RuntimeDir=Join-Path $repo '.runtime\lga-09-limited-ga-stability-confirmation-capacity-risk-review'
$wpfSalesVm=Join-Path $repo 'src\PosCore\SolidPOS.PosCore.Wpf\ViewModels\SalesViewModel.cs'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:Lga10ValidatorVersion"
Write-Step 'Repository/document LGA-10 guardrails...'
Assert-True (Test-Path $sln) 'solidpos-platform.sln missing';Assert-True (Test-Path $sqlPath) 'LGA-10 SQL check missing'
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-10-limited-ga-commercial-operations-confidence-gate.md') @('lga-09','commercial operations confidence','sales','payments','receipts','cash shifts','inventory','dashboard','audit','reports','support operations','public ga not activated','schema version 4')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-10-commercial-operations-confidence-plan.md') @('commercial operations confidence','/health/live','/health/ready','waiting connections','limited ga','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-10-commercial-confidence-gate-checklist.md') @('commercial confidence gate','concurrency 3','p95 readiness','negative stock','sync queues','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-10-evidence-matrix.md') @('evidence matrix','lga-09','schema version 4','commercial operations','decision')
...
Write-Step 'Repository/document LGA-10 guardrails PASS'

Write-Step 'LGA-09 prerequisite...'
if($SkipLga09Revalidation.IsPresent){Write-Step 'LGA-09 prerequisite revalidation skipped by switch; LGA-10 requires existing LGA-09 PASS logs and reviewed capacity-risk decision.'}
else{
  Assert-True (Test-Path $lga09RuntimeDir) 'LGA-09 runtime manifest directory missing. Rerun LGA-09 or use -SkipLga09Revalidation only if PASS logs are available.'
  $lga09Manifests=@(Get-ChildItem -Path $lga09RuntimeDir -Filter '*.json' -ErrorAction SilentlyContinue)
  $lga09Pass=$false
  foreach($f in $lga09Manifests){try{$m=Get-Content -Raw $f.FullName|ConvertFrom-Json;if(([string](Req $m 'status')).Contains('PASS LGA-09 LIMITED GA STABILITY CONFIRMATION')){$lga09Pass=$true}}catch{}}
  Assert-True $lga09Pass 'LGA-09 PASS manifest not found. LGA-10 requires LGA-09 closure.'
}
Write-Step 'LGA-09 prerequisite PASS'
...
Write-Step 'LGA-10 commercial operations API checks...'
...
Write-Step 'LGA-10 commercial operations API checks PASS'
...
Write-Step 'Database LGA-10 commercial operations snapshot...'
$db=Invoke-DbJsonFile $sqlPath @{tenant_id=$TenantId;max_stores=$MaxStores;max_concurrent_terminals=$MaxConcurrentTerminals;allowed_existing_sync_conflicts=$AllowedExistingSyncConflictCount;allowed_dead_letters=$AllowedDeadLetterCount;allowed_waiting_connections=$AllowedWaitingConnectionCount} 'LGA10_JSON:'
...
Write-Step 'Database LGA-10 commercial operations snapshot PASS'

Write-Step 'LGA-10 commercial operations confidence blocker matrix...'
