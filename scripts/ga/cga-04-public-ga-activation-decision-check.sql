...
WITH required_tables AS (
  SELECT unnest(ARRAY['pos.tenants','pos.tenant_configs','pos.stores','pos.users','pos.roles','pos.permissions','pos.user_roles','pos.user_store_access','pos.terminals','pos.categories','pos.products','pos.price_lists','pos.product_prices','pos.payment_methods','pos.sales','pos.payments','pos.customers','pos.cash_shifts','pos.inventory_ledger','pos.sync_inbox_events','pos.sync_conflicts','pos.audit_events','pos.update_releases','pos.digital_receipts','pos.return_refunds']) AS table_name
), table_presence AS (
  ...
  SELECT
    ...
    (SELECT COALESCE(sum(total_cents),0)::bigint FROM pos.sales WHERE tenant_id = :'tenant_uuid'::uuid AND status = 'completed' AND occurred_at >= now() - interval '24 hours') AS completed_sales_cents_24h,
    ...
    (SELECT COALESCE(sum(amount_cents),0)::bigint FROM pos.payments WHERE tenant_id = :'tenant_uuid'::uuid AND created_at >= now() - interval '24 hours') AS payment_cents_24h,
...
SELECT 'CGA04_JSON:' || result::text FROM payload;
