...
WITH required_tables AS (
  ...
  SELECT
    ...
    (SELECT count(*)::bigint FROM pos.cash_shifts WHERE tenant_id = :'tenant_uuid'::uuid AND status = 'closed') AS closed_shifts_total,
    (SELECT max(occurred_at) FROM pos.sales WHERE tenant_id = :'tenant_uuid'::uuid AND status = 'completed') AS latest_completed_sale_at,
    (SELECT max(created_at) FROM pos.payments WHERE tenant_id = :'tenant_uuid'::uuid) AS latest_payment_at
), payload AS (
  SELECT jsonb_build_object(
    ...
    'negativeStock', jsonb_build_object('count', (SELECT count(*) FROM negative_stock), 'items', COALESCE((SELECT jsonb_agg(row_to_json(negative_stock)) FROM negative_stock), '[]'::jsonb)),
...
SELECT 'PUBLIC_GA_READINESS_REVIEW_JSON:' || result::text FROM payload;
