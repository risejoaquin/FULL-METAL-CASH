...
WITH cfg AS (
  ...
  SELECT
    count(*)::bigint AS total_connection_count,
    count(*) FILTER (WHERE wait_event IS NOT NULL)::bigint AS legacy_wait_event_connection_count,
    count(*) FILTER (WHERE wait_event_type='Client')::bigint AS client_waiting_connection_count,
    count(*) FILTER (WHERE state='idle')::bigint AS idle_connection_count,
    count(*) FILTER (WHERE state='idle in transaction')::bigint AS idle_in_transaction_count,
    count(*) FILTER (WHERE state='active' AND wait_event IS NOT NULL AND COALESCE(wait_event_type,'') NOT IN ('Client','Activity'))::bigint AS waiting_connection_count,
...
SELECT 'FINAL_PUBLIC_GA_CLOSURE_JSON:'||result::text FROM payload;
