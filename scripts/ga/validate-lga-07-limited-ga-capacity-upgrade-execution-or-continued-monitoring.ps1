param(
    ...
    [switch]$SkipLga06Revalidation
)
$ErrorActionPreference='Stop'
$script:Lga07ValidatorVersion='LGA-07.0-limited-ga-capacity-upgrade-execution-or-continued-monitoring'
function Write-Step([string]$m){Write-Host "[LGA-07] $m"}
...
$sln=Join-Path $repo 'solidpos-platform.sln';$sqlPath=Join-Path $scriptRoot 'lga-07-limited-ga-capacity-upgrade-execution-or-continued-monitoring-check.sql'
...
$runtimeDir=Join-Path $repo '.runtime\lga-07-limited-ga-capacity-upgrade-execution-or-continued-monitoring';$lga05RuntimeDir=Join-Path $repo '.runtime\lga-06-limited-ga-operational-continuity-capacity-upgrade-preparation'
$wpfSalesVm=Join-Path $repo 'src\PosCore\SolidPOS.PosCore.Wpf\ViewModels\SalesViewModel.cs'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:Lga07ValidatorVersion"
Write-Step 'Repository/document LGA-07 guardrails...'
Assert-True (Test-Path $sln) 'solidpos-platform.sln missing';Assert-True (Test-Path $sqlPath) 'LGA-07 SQL check missing'
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-07-limited-ga-capacity-upgrade-execution-or-continued-monitoring.md') @('lga-06','operational continuity','capacity upgrade execution','continued monitoring','formal capacity risk accepted','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-07-capacity-upgrade-execution-or-continued-monitoring-plan.md') @('capacity upgrade execution','continued monitoring','railway pro','capacity remediation','rollback')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-07-capacity-monitoring-checklist.md') @('capacity monitoring','limited ga','public ga not activated','support')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-07-evidence-matrix.md') @('evidence matrix','lga-06','capacity upgrade','continued monitoring','decision')
Write-Step 'Repository/document LGA-07 guardrails PASS'

Write-Step 'LGA-06 operational continuity prerequisite...'
if($SkipLga06Revalidation.IsPresent){Write-Step 'LGA-06 prerequisite revalidation skipped by switch; LGA-07 requires existing LGA-06 PASS logs with CONTINUE LIMITED GA.'}
else{
  Assert-True (Test-Path $lga05RuntimeDir) 'LGA-06 runtime manifest directory missing. Rerun LGA-06 continuation or use -SkipLga06Revalidation only if PASS logs are available.'
  $lga04Manifests=@(Get-ChildItem -Path $lga05RuntimeDir -Filter '*.json' -ErrorAction SilentlyContinue)
  $lga04Pass=$false
  foreach($f in $lga04Manifests){try{$m=Get-Content -Raw $f.FullName|ConvertFrom-Json;if(([string](Req $m 'status')).Contains('PASS LGA-06 LIMITED GA OPERATIONAL CONTINUITY AND CAPACITY UPGRADE PREPARATION / CONTINUE LIMITED GA')){$lga04Pass=$true}}catch{}}
  Assert-True $lga04Pass 'LGA-06 continuation PASS manifest not found. LGA-07 requires LGA-06 formal capacity risk acceptance.'
}
Write-Step 'LGA-06 operational continuity prerequisite PASS'
...
$db=Invoke-DbJsonFile $sqlPath @{tenant_id=$TenantId;max_stores=$MaxStores;max_concurrent_terminals=$MaxConcurrentTerminals;allowed_existing_sync_conflicts=$AllowedExistingSyncConflictCount;allowed_dead_letters=$AllowedDeadLetterCount;allowed_waiting_connections=$AllowedWaitingConnectionCount} 'LGA07_JSON:'
...
Write-Step 'LGA-07 limited GA capacity upgrade execution or continued monitoring blocker matrix...'
