...
WITH required_tables AS (
  ...
  SELECT
    ...
    (SELECT count(*)::bigint FROM pos.price_lists WHERE tenant_id = :'tenant_uuid'::uuid AND status = 'active' AND deleted_at IS NULL) AS active_price_list_count,
  ...
  SELECT
    ...
    (SELECT count(*)::bigint FROM pos.roles WHERE tenant_id = :'tenant_uuid'::uuid AND deleted_at IS NULL) AS role_count,
    (SELECT count(*)::bigint FROM pos.permissions) AS permission_count,
    (SELECT count(*)::bigint FROM pos.user_roles WHERE tenant_id = :'tenant_uuid'::uuid) AS user_role_assignment_count,
    (SELECT count(*)::bigint FROM pos.user_store_access WHERE tenant_id = :'tenant_uuid'::uuid) AS user_store_access_count
), customer_readiness AS (
  SELECT
    (SELECT count(*)::bigint FROM pos.customers WHERE tenant_id = :'tenant_uuid'::uuid AND deleted_at IS NULL) AS customer_count,
    (SELECT count(*)::bigint FROM pos.digital_receipts WHERE tenant_id = :'tenant_uuid'::uuid AND status = 'active') AS active_digital_receipt_count,
    (SELECT count(*)::bigint FROM pos.sales WHERE tenant_id = :'tenant_uuid'::uuid AND status IN ('completed','partially_returned','returned')) AS completed_sale_count
), integrity AS (
  SELECT
  ...
  SELECT jsonb_build_object(
    ...
    'operatorReadiness', (SELECT row_to_json(operator_readiness) FROM operator_readiness),
    'adminReadiness', (SELECT row_to_json(admin_readiness) FROM admin_readiness),
    'customerReadiness', (SELECT row_to_json(customer_readiness) FROM customer_readiness),
...
SELECT 'GA11_JSON:' || result::text FROM payload;
