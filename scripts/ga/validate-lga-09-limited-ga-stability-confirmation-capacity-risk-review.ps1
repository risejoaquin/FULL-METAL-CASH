param(
    ...
    [ValidateSet('CONTINUE_LIMITED_GA','CAPACITY_UPGRADE_REQUIRED_BEFORE_PUBLIC_GA')][string]$StabilityDecision = 'CAPACITY_UPGRADE_REQUIRED_BEFORE_PUBLIC_GA',
    ...
    [switch]$SkipLga08Revalidation
)
$ErrorActionPreference='Stop'
$script:Lga09ValidatorVersion='LGA-09.0-limited-ga-stability-confirmation-capacity-risk-review'
function Write-Step([string]$m){Write-Host "[LGA-09] $m"}
...
$sln=Join-Path $repo 'solidpos-platform.sln';$sqlPath=Join-Path $scriptRoot 'lga-09-limited-ga-stability-confirmation-capacity-risk-review-check.sql'
...
$runtimeDir=Join-Path $repo '.runtime\lga-09-limited-ga-stability-confirmation-capacity-risk-review';$lga08RuntimeDir=Join-Path $repo '.runtime\lga-08-limited-ga-post-upgrade-verification-or-continued-monitoring'
$wpfSalesVm=Join-Path $repo 'src\PosCore\SolidPOS.PosCore.Wpf\ViewModels\SalesViewModel.cs'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:Lga09ValidatorVersion"
Write-Step 'Repository/document LGA-09 guardrails...'
Assert-True (Test-Path $sln) 'solidpos-platform.sln missing';Assert-True (Test-Path $sqlPath) 'LGA-09 SQL check missing'
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-09-limited-ga-stability-confirmation-capacity-risk-review.md') @('lga-08','stability confirmation','capacity risk review','public ga not activated','schema version 4')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-09-stability-confirmation-plan.md') @('stability confirmation','/health/live','/health/ready','waiting connections','limited ga','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-09-capacity-risk-review-checklist.md') @('capacity risk review','concurrency 3','p95 readiness','negative stock','sync queues','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-09-evidence-matrix.md') @('evidence matrix','lga-08','schema version 4','capacity','decision')
Write-Step 'Repository/document LGA-09 guardrails PASS'

Write-Step 'LGA-08 prerequisite...'
if($SkipLga08Revalidation.IsPresent){Write-Step 'LGA-08 prerequisite revalidation skipped by switch; LGA-09 requires existing LGA-08 PASS logs with CONTINUE LIMITED GA.'}
else{
  Assert-True (Test-Path $lga08RuntimeDir) 'LGA-08 runtime manifest directory missing. Rerun LGA-08 or use -SkipLga08Revalidation only if PASS logs are available.'
  $lga08Manifests=@(Get-ChildItem -Path $lga08RuntimeDir -Filter '*.json' -ErrorAction SilentlyContinue)
  $lga08Pass=$false
  foreach($f in $lga08Manifests){try{$m=Get-Content -Raw $f.FullName|ConvertFrom-Json;if(([string](Req $m 'status')).Contains('PASS LGA-08 LIMITED GA POST-UPGRADE VERIFICATION OR CONTINUED MONITORING')){$lga08Pass=$true}}catch{}}
  Assert-True $lga08Pass 'LGA-08 PASS manifest not found. LGA-09 requires LGA-08 closure.'
}
Write-Step 'LGA-08 prerequisite PASS'
...
Write-Step 'LGA-09 stability confirmation API checks...'
...
Write-Step 'LGA-09 stability confirmation API checks PASS'
...
Write-Step 'Database LGA-09 stability snapshot...'
$db=Invoke-DbJsonFile $sqlPath @{tenant_id=$TenantId;max_stores=$MaxStores;max_concurrent_terminals=$MaxConcurrentTerminals;allowed_existing_sync_conflicts=$AllowedExistingSyncConflictCount;allowed_dead_letters=$AllowedDeadLetterCount;allowed_waiting_connections=$AllowedWaitingConnectionCount} 'LGA09_JSON:'
...
Write-Step 'Database LGA-09 stability snapshot PASS'

Write-Step 'LGA-09 stability confirmation / capacity risk blocker matrix...'
