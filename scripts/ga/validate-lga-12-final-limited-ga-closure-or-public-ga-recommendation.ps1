param(
    ...
    [ValidateSet('CONTINUE_LIMITED_GA','RECOMMEND_PUBLIC_GA_REVIEW','BLOCK_PUBLIC_GA_UNTIL_CAPACITY_UPGRADE')][string]$FinalDecision = 'CONTINUE_LIMITED_GA',
    [ValidateSet('CAPACITY_UPGRADE_REQUIRED_BEFORE_PUBLIC_GA','CAPACITY_GATE_PASSED')][string]$CapacityRecommendation = 'CAPACITY_UPGRADE_REQUIRED_BEFORE_PUBLIC_GA',
    ...
    [switch]$SkipLga11Revalidation
)
$ErrorActionPreference='Stop'
$script:Lga12ValidatorVersion='LGA-12.0-final-limited-ga-closure-or-public-ga-recommendation'
function Write-Step([string]$m){Write-Host "[LGA-12] $m"}
...
$sln=Join-Path $repo 'solidpos-platform.sln';$sqlPath=Join-Path $scriptRoot 'lga-12-final-limited-ga-closure-or-public-ga-recommendation-check.sql'
...
$runtimeDir=Join-Path $repo '.runtime\lga-12-final-limited-ga-closure-or-public-ga-recommendation';$lga11RuntimeDir=Join-Path $repo '.runtime\lga-11-limited-ga-public-ga-readiness-reassessment'
$wpfSalesVm=Join-Path $repo 'src\PosCore\SolidPOS.PosCore.Wpf\ViewModels\SalesViewModel.cs'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:Lga12ValidatorVersion"
Write-Step 'Repository/document LGA-12 guardrails...'
Assert-True (Test-Path $sln) 'solidpos-platform.sln missing';Assert-True (Test-Path $sqlPath) 'LGA-12 SQL check missing'
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-12-final-limited-ga-closure-or-public-ga-recommendation.md') @('lga-11','final limited ga closure','public ga recommendation','capacity','keep limited ga','public ga not activated','schema version 4')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-12-final-decision-plan.md') @('final decision','continue limited ga','capacity upgrade required before public ga','public ga not activated','/health/live','/health/ready')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-12-final-closure-checklist.md') @('closure checklist','concurrency 3','p95 readiness','negative stock','sync queues','commercial operations','public ga not activated')
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-12-evidence-matrix.md') @('evidence matrix','lga-11','schema version 4','capacity','final decision')
Assert-DocumentContains (Join-Path $repo 'docs\ga\ga-03-support-incident-slo-operations-readiness.md') @('support','incident')
Assert-DocumentContains (Join-Path $repo 'docs\ga\ga-10-oncall-dashboard-runbook.md') @('dashboard','on-call')
Write-Step 'Repository/document LGA-12 guardrails PASS'

Write-Step 'LGA-11 prerequisite...'
if($SkipLga11Revalidation.IsPresent){Write-Step 'LGA-11 prerequisite revalidation skipped by switch; LGA-12 requires existing LGA-11 PASS logs and reviewed readiness decision.'}
else{
  Assert-True (Test-Path $lga11RuntimeDir) 'LGA-11 runtime manifest directory missing. Rerun LGA-11 or use -SkipLga11Revalidation only if reviewed PASS logs are available.'
  $lga11Manifests=@(Get-ChildItem -Path $lga11RuntimeDir -Filter '*.json' -ErrorAction SilentlyContinue)
  $lga11Pass=$false
  foreach($f in $lga11Manifests){try{$m=Get-Content -Raw $f.FullName|ConvertFrom-Json;if(([string](Req $m 'status')).Contains('PASS LGA-11 LIMITED GA PUBLIC GA READINESS REASSESSMENT')){$lga11Pass=$true}}catch{}}
  Assert-True $lga11Pass 'LGA-11 PASS manifest not found. LGA-12 requires LGA-11 closure.'
}
Write-Step 'LGA-11 prerequisite PASS'
...
Write-Step 'LGA-12 final closure/recommendation API checks...'
...
Write-Step 'LGA-12 final closure/recommendation API checks PASS'
...
Write-Step 'Database LGA-12 final closure/recommendation snapshot...'
$db=Invoke-DbJsonFile $sqlPath @{tenant_id=$TenantId;max_stores=$MaxStores;max_concurrent_terminals=$MaxConcurrentTerminals;allowed_existing_sync_conflicts=$AllowedExistingSyncConflictCount;allowed_dead_letters=$AllowedDeadLetterCount;allowed_waiting_connections=$AllowedWaitingConnectionCount} 'LGA12_JSON:'
...
Write-Step 'Database LGA-12 final closure/recommendation snapshot PASS'

Write-Step 'LGA-12 final closure/recommendation blocker matrix...'
...
if((-not $capacityProbePassed) -and $CapacityRecommendation -ne 'CAPACITY_UPGRADE_REQUIRED_BEFORE_PUBLIC_GA'){Add-Blocker 'capacity_recommendation_must_require_upgrade_before_public_ga' @{capacityProbePassed=$capacityProbePassed;capacityRecommendation=$CapacityRecommendation}}
if($capacityProbePassed -and $CapacityRecommendation -ne 'CAPACITY_GATE_PASSED'){Add-Blocker 'capacity_recommendation_must_record_passed_gate' @{capacityProbePassed=$capacityProbePassed;capacityRecommendation=$CapacityRecommendation}}
if((-not $capacityProbePassed) -and $FinalDecision -notin @('CONTINUE_LIMITED_GA','BLOCK_PUBLIC_GA_UNTIL_CAPACITY_UPGRADE')){Add-Blocker 'final_decision_cannot_recommend_public_ga_while_capacity_fails' @{finalDecision=$FinalDecision;capacityProbePassed=$capacityProbePassed}}
if($FinalDecision -eq 'RECOMMEND_PUBLIC_GA_REVIEW' -and -not $capacityProbePassed){Add-Blocker 'public_ga_review_recommendation_requires_capacity_gate_pass' $FinalDecision}
if($capacityProbePassed -and $FinalDecision -eq 'BLOCK_PUBLIC_GA_UNTIL_CAPACITY_UPGRADE'){Add-Blocker 'capacity_upgrade_block_not_valid_after_capacity_gate_pass' $FinalDecision}
if($PublicGaDecision -ne 'KEEP_LIMITED_GA'){Add-Blocker 'public_ga_decision_must_keep_limited_ga' $PublicGaDecision}
Add-Condition 'public_ga_not_activated';Add-Condition 'lga11_closed';Add-Condition 'limited_ga_only';Add-Condition ('final_decision_' + $FinalDecision.ToLowerInvariant());Add-Condition ('capacity_recommendation_' + $CapacityRecommendation.ToLowerInvariant());Add-Condition 'schema_version_4_required';Add-Condition 'negative_stock_zero_required';Add-Condition 'waiting_connections_baseline_not_raised';Add-Condition 'sync_conflict_and_dead_letter_baselines_carried_forward';Add-Condition 'lga12_does_not_activate_public_ga';Add-Condition 'final_capacity_risk_recorded'
Assert-True ($blockers.Count -eq 0) ("LGA-12 blockers present: " + ($blockers|ConvertTo-Json -Depth 20 -Compress))
Write-Step 'LGA-12 final closure/recommendation blocker matrix PASS'

$status=if($FinalDecision -eq 'RECOMMEND_PUBLIC_GA_REVIEW'){'PASS LGA-12 FINAL LIMITED GA CLOSURE / PUBLIC GA REVIEW RECOMMENDATION ONLY - NOT ACTIVATED'}elseif($FinalDecision -eq 'BLOCK_PUBLIC_GA_UNTIL_CAPACITY_UPGRADE'){'PASS LGA-12 FINAL LIMITED GA CLOSURE / PUBLIC GA BLOCKED UNTIL CAPACITY UPGRADE'}else{'PASS LGA-12 FINAL LIMITED GA CLOSURE / CONTINUE LIMITED GA / PUBLIC GA NOT ACTIVATED / CAPACITY UPGRADE REQUIRED BEFORE PUBLIC GA'}
$nextPhase='LIMITED_GA_ROADMAP_COMPLETE_REVIEW_REQUIRED'
$manifest=[ordered]@{phase='LGA-12';status=$status;tenantId=$TenantId;baseUrl=$BaseUrl;dashboardUrl=$DashboardUrl;generatedAt=(Get-Date).ToUniversalTime().ToString('o');checkpointDate=(Get-Date).ToString('yyyy-MM-dd');validatorVersion=$script:Lga12ValidatorVersion;entryGate='PASS LGA-11 LIMITED GA PUBLIC GA READINESS REASSESSMENT';finalDecision=$FinalDecision;capacityRecommendation=$CapacityRecommendation;conflictDecision=$ConflictDecision;deadLetterDecision=$DeadLetterDecision;capacityDecision=$CapacityDecision;publicGaDecision=$PublicGaDecision;formalCapacityRiskAccepted=($CapacityDecision -eq 'FORMAL_ACCEPT_LIMITED_CAPACITY');maxStores=$MaxStores;maxConcurrentTerminals=$MaxConcurrentTerminals;publicGaReadinessConcurrency=$PublicGaReadinessConcurrency;concurrencyProbeRequests=$ConcurrencyProbeRequests;maxReadinessP95Ms=$MaxReadinessP95Ms;capacityProbePassed=$capacityProbePassed;healthLiveConcurrencyProbe=$liveProbe;healthReadyConcurrencyProbe=$readyProbe;minCompletedSalesInLast24h=$MinCompletedSalesInLast24h;minPaymentsInLast24h=$MinPaymentsInLast24h;minReceiptsIssuedInLast24h=$MinReceiptsIssuedInLast24h;minAuditEventsInLast24h=$MinAuditEventsInLast24h;minClosedShiftsTotal=$MinClosedShiftsTotal;allowedCashDifferenceLast24hCount=$AllowedCashDifferenceLast24hCount;allowedExistingSyncConflictCount=$AllowedExistingSyncConflictCount;allowedDeadLetterCount=$AllowedDeadLetterCount;allowedNegativeStockItemCount=$AllowedNegativeStockItemCount;allowedOpenShiftCount=$AllowedOpenShiftCount;allowedWaitingConnectionCount=$AllowedWaitingConnectionCount;publicGeneralAvailabilityActivated=$false;generalAvailabilityActivated=$false;publicGaActivatedByValidator=$false;healthLiveStatus=$healthLiveStatus;healthReadyStatus=$healthReadyStatus;unauthenticatedObservabilityStatus=$unauthObservabilityStatus;inventoryStockStatus=$inventoryStatus;salesRangeCompletedSalesCount=[int](Req $salesRange 'completedSalesCount');dashboardOverviewCompletedSalesCount=[int](Req (Req $dashboardOverview 'sales') 'completedSalesCount');completedSalesInLast24h=$completedSales24h;paymentsInLast24h=$payments24h;receiptsIssuedInLast24h=$receiptsIssued24h;auditEventsInLast24h=$auditEvents24h;syncPendingCount=$syncPendingCount;syncProcessingCount=$syncProcessingCount;syncRetryPendingCount=$syncRetryPendingCount;syncConflictCount=$syncConflictCount;syncDeadLetterCount=$syncDeadLetterCount;dashboardUrlStatus=$dashboardUrlStatus;dashboardBuild=if($SkipDashboardBuild.IsPresent){'SKIPPED_BY_SWITCH'}else{'VALIDATED'};metricsDatabaseReady=$metricsDatabaseReady;syncContractCurrentSchemaVersion=[int]$contractSchema;databaseSnapshot=$db;negativeStockCount=$negativeStockCount;negativeStock=$negativeStock;activeStoreCount=$activeStoreCount;availableTerminalCount=$availableTerminalCount;openShiftCount=$openShiftCount;closedShiftCount=$closedShiftCount;cashDifferenceLast24hCount=$cashDifferenceLast24hCount;activeStableReleaseCount=$activeStableReleaseCount;duplicateLocalSaleCount=$duplicateLocalSaleCount;legacySchemaEventCount=$legacySchemaEventCount;pendingConflictCount=$pendingConflictCount;retryPendingCount=$retryPendingCount;deadLetterCount=$deadLetterCount;staleProcessingCount=$staleProcessingCount;rlsMissingTableCount=$rlsMissingTableCount;waitingConnectionCount=$waitingConnectionCount;longRunningQueryCount=$longRunningQueryCount;wpfQsrVisualConfirmed=$WpfVisualConfirmed.IsPresent;blockerCount=$blockers.Count;blockers=$blockers;conditions=@($conditions);schemaVersion=4;syncContract='schema_version_4';publicGaActivation='NOT_ACTIVATED';nextPhase=$nextPhase}
$manifestPath=Join-Path $runtimeDir ("lga-12-final-limited-ga-closure-or-public-ga-recommendation-" + (Get-Date).ToString('yyyy-MM-dd-HHmmss') + ".json");$manifest|ConvertTo-Json -Depth 100|Set-Content -Encoding UTF8 $manifestPath
Write-Step 'LGA-12 evidence manifest and final closure/recommendation snapshot PASS'
Write-Step $status
