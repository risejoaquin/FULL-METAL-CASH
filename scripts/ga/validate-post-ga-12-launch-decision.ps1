param(
    ...
    [Parameter(Mandatory=$true)][ValidateSet('CONTROLLED_ROLLOUT','CAPACITY_SCALE_UP','NO_GO_REMEDIATION')][string]$Decision,
    [switch]$SkipDashboardBuild,
    [switch]$SkipGa12Revalidation
)

$ErrorActionPreference = 'Stop'
$script:PostGa12ValidatorVersion = 'POST-GA-12.0-launch-decision-control-plane'
function Write-Step([string]$message) { Write-Host "[POST-GA-12] $message" }
...
function Invoke-DbJsonFile([string]$SqlPath, [hashtable]$Vars) {
    ...
    foreach ($candidate in @($output | ForEach-Object { [string]$_ })) {
        ...
        $line = $candidate.Trim()
        if ($line.StartsWith('POST_GA12_JSON:')) { $line = $line.Substring('POST_GA12_JSON:'.Length) }
        if (-not $line.StartsWith('{')) { continue }
        try { return $line | ConvertFrom-Json } catch {}
    }
    $marker = 'POST_GA12_JSON:'
...
$sqlPath = Join-Path $scriptRoot 'post-ga-12-launch-decision-check.sql'
$ga12Script = Join-Path $scriptRoot 'validate-ga-12-final-general-availability-launch-readiness.ps1'
$ga12Manifest = Join-Path $repo '.runtime\ga-12-final-general-availability-launch-readiness\ga-12-final-general-availability-launch-readiness-manifest.json'
...
$runtime = Join-Path $repo '.runtime\post-ga-12-launch-decision'
$manifestPath = Join-Path $runtime 'post-ga-12-launch-decision-manifest.json'
$snapshotPath = Join-Path $runtime 'post-ga-12-launch-decision-snapshot.json'
$evidencePath = Join-Path $runtime 'post-ga-12-launch-decision-evidence.md'
$logPath = Join-Path $repo 'docs\ga\logs\post-ga-12-launch-decision-log.md'
New-Item -ItemType Directory -Force -Path @($runtime, (Split-Path $logPath)) | Out-Null

Write-Step "Validator version $script:PostGa12ValidatorVersion"
...
Assert-True($Decision -in @('CONTROLLED_ROLLOUT','CAPACITY_SCALE_UP','NO_GO_REMEDIATION')) 'Decision must be CONTROLLED_ROLLOUT, CAPACITY_SCALE_UP, or NO_GO_REMEDIATION.'

Write-Step 'Repository/document Post-GA-12 guardrails...'
Assert-True(Test-Path $sln) 'solution missing'
Assert-True(Test-Path $sqlPath) 'Post-GA-12 SQL check missing'
Assert-True(Test-Path $ga12Script) 'GA-12 prerequisite validator missing'
...
$docs = @(
    ...
    (Join-Path $repo 'SOLIDPOS_POST_GA_12_LAUNCH_DECISION.md'),
    (Join-Path $repo 'docs\ga\post-ga-12-launch-decision.md'),
    (Join-Path $repo 'docs\ga\post-ga-12-decision-record.md'),
    (Join-Path $repo 'docs\ga\post-ga-12-controlled-rollout-plan.md'),
    (Join-Path $repo 'docs\ga\post-ga-12-capacity-scale-up-plan.md'),
    (Join-Path $repo 'docs\ga\post-ga-12-no-go-remediation-plan.md')
)
foreach($doc in $docs){ Assert-True(Test-Path $doc) "Required Post-GA-12 document missing: $doc" }
Assert-DocumentContains $docs[0] @('GA-12','PASS GENERAL AVAILABILITY READINESS / GO CONTROLLED GA ROLLOUT','POST-GA-12','GENERAL AVAILABILITY: NOT ACTIVATED')
Assert-DocumentContains $docs[2] @('Post-GA-12 Launch Decision','CONTROLLED_ROLLOUT','CAPACITY_SCALE_UP','NO_GO_REMEDIATION','does not activate','publicGeneralAvailabilityActivated=False')
Assert-DocumentContains $docs[3] @('GO_CONTROLLED_GA_ROLLOUT','public_ga_activation_requires_explicit_post_ga12_decision','does not flip')
Assert-DocumentContains $docs[4] @('controlled rollout','rollback','capacity boundary','waiting connections')
Assert-DocumentContains $docs[5] @('capacity scale-up','Concurrency 3+','Railway','database connections')
Assert-DocumentContains $docs[6] @('NO_GO_REMEDIATION','blockers','remediation')
Write-Step 'Repository/document Post-GA-12 guardrails PASS'
...
if (-not $SkipGa12Revalidation.IsPresent) {
    Write-Step 'Revalidating GA-12 prerequisite...'
    $ga12Args = @{ BaseUrl=$script:base; DashboardUrl=$DashboardUrl; TenantId=$TenantId; Email=$Email; Password=$Password; DatabaseUrl=$DatabaseUrl; SkipDashboardBuild=$SkipDashboardBuild; SkipGa11Revalidation=$true }
    & $ga12Script @ga12Args
    Write-Step 'GA-12 prerequisite revalidation PASS'
} else {
    Write-Step 'GA-12 prerequisite revalidation skipped by switch; Post-GA-12 requires existing GA-12 PASS logs or runtime manifest.'
    if (Test-Path $ga12Manifest) {
        $ga12 = Get-Content -Raw $ga12Manifest | ConvertFrom-Json
        Assert-True(([string]$ga12.status).Contains('PASS GENERAL AVAILABILITY READINESS / GO CONTROLLED GA ROLLOUT')) 'GA-12 manifest status is not PASS / GO CONTROLLED GA ROLLOUT.'
        Assert-True(-not [bool]$ga12.generalAvailabilityActivated) 'GA-12 manifest indicates GA activated.'
        Assert-True([string]$ga12.finalDecision -eq 'GO_CONTROLLED_GA_ROLLOUT') 'GA-12 finalDecision was not GO_CONTROLLED_GA_ROLLOUT.'
    }
}

Write-Step 'Production post-GA-12 decision API checks...'
...
Write-Step 'Production post-GA-12 decision API checks PASS'

Write-Step 'Database post-GA-12 decision snapshot...'
...
Assert-True([bool]$sql.requiredTablesPresent) "Missing required launch decision source tables: $($sql.missingRequiredTables -join ', ')"
Assert-True(-not [bool]$sql.generalAvailabilityActivated) 'Post-GA-12 SQL indicates General Availability already activated.'
Assert-True([long]$sql.tenantState.active_tenant_count -eq 1) 'Tenant must be active for Post-GA-12.'
...
Write-Step 'Database post-GA-12 decision snapshot PASS'

Write-Step 'Post-GA-12 launch decision matrix...'
$blockers = @()
$conditions = @(
...
if ($SkipGa12Revalidation.IsPresent) { $conditions += 'ga12_revalidation_skipped_requires_external_ga12_pass_log' }
...
if ([bool]$sql.requiredTablesPresent -ne $true) { $blockers += 'launch_decision_source_tables_missing' }
...
Assert-True($blockers.Count -eq 0) "POST-GA-12 BLOCKED: $($blockers -join ', ')"

$status = switch ($Decision) {
    'CONTROLLED_ROLLOUT' { 'PASS POST-GA-12 CONTROLLED ROLLOUT DECISION / READY FOR LIMITED GA EXECUTION' }
    'CAPACITY_SCALE_UP' { 'PASS POST-GA-12 CAPACITY SCALE-UP DECISION / SCALE BEFORE PUBLIC GA' }
    'NO_GO_REMEDIATION' { 'PASS POST-GA-12 NO-GO REMEDIATION DECISION / FIX BEFORE LAUNCH EXPANSION' }
}
Write-Step 'Post-GA-12 launch decision matrix PASS'

$logoutStatus = Get-HttpStatus Post '/api/v1/auth/logout' $headers @{ refreshToken = [string]$login.refreshToken; tenantId = $TenantId }
Assert-True($logoutStatus -eq 204) "Post-GA-12 session logout must return 204; status=$logoutStatus"
...
$manifest = [ordered]@{
    ...
    validatorVersion = $script:PostGa12ValidatorVersion
    entryGate = 'PASS GENERAL AVAILABILITY READINESS / GO CONTROLLED GA ROLLOUT'
    selectedDecision = $Decision
    controlledRolloutAllowed = ($Decision -eq 'CONTROLLED_ROLLOUT')
    capacityScaleUpSelected = ($Decision -eq 'CAPACITY_SCALE_UP')
    noGoRemediationSelected = ($Decision -eq 'NO_GO_REMEDIATION')
    ...
    knownDbCondition = 'GA-10/GA-11/GA-12 observed waiting connections; monitor/tune pool/connections before public GA launch.'
    ...
    nextStep = switch ($Decision) {
        'CONTROLLED_ROLLOUT' { 'Execute limited controlled GA rollout only; keep public GA activation disabled until capacity and DB conditions are accepted or remediated.' }
        'CAPACITY_SCALE_UP' { 'Scale Railway/upstream/database capacity, rerun GA-09 concurrency profile, then rerun Post-GA-12 decision.' }
        'NO_GO_REMEDIATION' { 'Open remediation workstream for blockers/conditions, then rerun GA-12 and Post-GA-12.' }
    }
}
...
@"
# Post-GA-12 Launch Decision Evidence
...
@"
# Post-GA-12 Launch Decision Log
...
$plainPassword = $null
[GC]::Collect()
Write-Step 'Post-GA-12 evidence manifest and launch decision snapshot PASS'
Write-Step $status
