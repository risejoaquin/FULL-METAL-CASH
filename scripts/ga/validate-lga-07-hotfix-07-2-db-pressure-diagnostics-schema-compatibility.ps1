...
$script:ValidatorVersion='LGA-07.2-db-pressure-diagnostics-schema-compatibility-no-public-ga'
function Write-Step([string]$m){Write-Host "[LGA-07.2] $m"}
...
function Invoke-ApiLogin([string]$Base,[string]$Tenant,[string]$User,[string]$PlainPassword){
  ...
  try{return Invoke-RestMethod -Method Post -Uri "$Base/api/v1/auth/login" -ContentType 'application/json' -Body $body -TimeoutSec 30}catch{throw "Login failed for LGA-07.2 diagnostics. $($_.Exception.Message)"}
}
...
$sqlPath=Join-Path $scriptRoot 'lga-07-hotfix-07-2-db-pressure-diagnostics-schema-compatibility-check.sql'
$runtimeDir=Join-Path $repo '.runtime\lga-07-hotfix-07-2-db-pressure-diagnostics-schema-compatibility'
New-Item -ItemType Directory -Force -Path $runtimeDir | Out-Null

Write-Step "Validator version $script:ValidatorVersion"
Write-Step 'Repository/document HOTFIX LGA-07.2 guardrails...'
Assert-True (Test-Path (Join-Path $repo 'solidpos-platform.sln')) 'solidpos-platform.sln missing'
Assert-True (Test-Path $sqlPath) 'LGA-07.2 diagnostic SQL missing'
Assert-DocumentContains (Join-Path $repo 'docs\ga\lga-07-hotfix-07-2-db-pressure-diagnostics-schema-compatibility.md') @('lga-07.1','db pressure','waiting connections','diagnostic only','public ga not activated','no baseline increase')
Write-Step 'Repository/document HOTFIX LGA-07.2 guardrails PASS'
...
$db=Invoke-DbJsonFile $sqlPath @{tenant_id=$TenantId} 'LGA072_DIAGNOSTIC_JSON:'
...
$manifest=[ordered]@{
  phase='LGA-07.2'
  status='PASS HOTFIX LGA-07.2 DB PRESSURE DIAGNOSTICS / LGA-07 REMAINS PENDING WHEN WAITING CONNECTIONS EXCEED BASELINE'
...
$manifestPath=Join-Path $runtimeDir ("lga-07-hotfix-07-2-db-pressure-diagnostics-schema-compatibility-" + (Get-Date).ToString('yyyy-MM-dd-HHmmss') + '.json')
