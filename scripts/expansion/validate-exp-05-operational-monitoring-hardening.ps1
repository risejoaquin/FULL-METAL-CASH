...
$blockers=@(); if($live.status -ne 'alive'){$blockers+='liveness_not_alive'}; if($ready.status -ne 'ready'){$blockers+='readiness_not_ready'}; if($ready.database -ne 'ready'){$blockers+='database_not_ready'}; if($pendingConflicts -gt 0){$blockers+='pending_conflicts'}; if($failedPayments24 -gt 0){$blockers+='failed_payments_last_24h'}; if($failedRequests -gt 0){$blockers+='failed_requests'}; if(($p95LatencyMs -gt 0) -and ($p95LatencyMs -gt $MaxP95LatencyMs)){$blockers+='p95_latency_over_threshold'}
$conditions=@(); if($retryPending -gt 0){$conditions+='monitor_retry_pending_sync'}; if($deadLetterCount -gt 0){$conditions+='triage_known_dead_letter'}; if($negativeInventory -gt 0){$conditions+='inventory_reconciliation_required'}; if($sql.openShiftCount -gt 0){$conditions+='review_open_cash_shifts'}
