WITH params AS (
  ...
  SELECT to_regclass('pos.tenants') IS NOT NULL AS has_tenants,
         ...
         to_regclass('pos.inventory_stock') IS NOT NULL AS has_inventory_stock,
  ...
  SELECT p.*,
         (SELECT count(*) FROM pos.tenants t WHERE t.id = p.tenant_id) AS tenant_exists_count,
         (SELECT count(*) FROM pos.stores s WHERE s.tenant_id = p.tenant_id AND s.id = p.store_id AND s.status = 'active') AS store_exists_count,
         (SELECT count(*) FROM pos.terminals tm WHERE tm.tenant_id = p.tenant_id AND tm.store_id = p.store_id AND tm.id = p.terminal_id AND tm.status = 'active') AS terminal_exists_count,
         ...
         (SELECT count(*) FROM pos.cash_shifts cs WHERE cs.tenant_id = p.tenant_id AND cs.id = p.shift_id AND cs.terminal_id = p.terminal_id AND cs.status = 'closed' AND cs.difference_cents = 0) AS closed_shift_count,
         ...
         (SELECT count(*) FROM pos.inventory_ledger il WHERE il.tenant_id = p.tenant_id AND il.reference_type = 'sale' AND il.reference_id::text = p.sale_id::text AND il.quantity_delta < 0) AS inventory_ledger_count,
  ...
  SELECT b.*, t.*,
         (t.has_tenants AND t.has_stores AND t.has_terminals AND t.has_sales AND t.has_sale_lines AND t.has_payments AND t.has_cash_shifts AND t.has_digital_receipts AND t.has_inventory_ledger AND t.has_inventory_stock AND t.has_sync_inbox_events AND t.has_sync_conflicts AND t.has_audit_events) AS required_tables_present
  FROM base b CROSS JOIN tables t
), reasons AS (
  SELECT array_remove(ARRAY[
    CASE WHEN NOT required_tables_present THEN 'required_table_missing' END,
    CASE WHEN tenant_exists_count <> 1 THEN 'tenant_missing' END,
    CASE WHEN store_exists_count <> 1 THEN 'store_missing_or_inactive' END,
    CASE WHEN terminal_exists_count <> 1 THEN 'second_terminal_missing_or_inactive' END,
    CASE WHEN sale_exists_count <> 1 THEN 'second_terminal_sale_missing_or_mismatched' END,
    ...
    CASE WHEN terminal_pending_conflict_count > 0 THEN 'second_terminal_pending_conflict' END,
    CASE WHEN terminal_dead_letter_count > 0 THEN 'second_terminal_dead_letter' END
...
SELECT json_build_object(
  ...
  'schemaVersion', 4,
  'exp03SqlValidation', CASE WHEN cardinality(blocking_reasons) = 0 THEN 'GO' ELSE 'NO-GO' END
) FROM reasons;
